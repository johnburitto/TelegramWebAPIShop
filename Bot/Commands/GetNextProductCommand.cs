...
{
    public class GetNextProductCommand : Command
    {
        protected override List<string> Names { get; set; } = new() { "next" };
        ...
        public override async Task Execute(ITelegramBotClient client, CallbackQuery callbackQuery)
        {
            ...
            var response = await RequestClient.Client.GetAsync($"api/Product/{id}/next");
