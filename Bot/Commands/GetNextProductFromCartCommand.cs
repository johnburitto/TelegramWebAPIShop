...
{
    public class GetNextProductFromCartCommand : Command
    {
        protected override List<string> Names { get; set; } = new() { "cart_nxt" };
        ...
        public override async Task Execute(ITelegramBotClient client, CallbackQuery callbackQuery)
        {
            ...
            var productId = cartDictionary?.GetNextProduct(int.Parse(id));
